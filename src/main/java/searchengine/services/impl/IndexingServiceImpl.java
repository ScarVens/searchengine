...
public class IndexingServiceImpl implements IndexingService {
    ...
    public boolean urlIndexing(String url) {
        if (urlCheck(url)) {
            log.info("Site reindexation started - " + url);
            executorService = Executors.newFixedThreadPool(coreCount);
            executorService.submit(new SiteIndex(pageRepository, siteRepository, lemmaRepository, indexSearchRepository, lemmaParser, indexParser, url, sitesList));
    ...
    public boolean stopIndexing() {
        if (isIndexingActive()) {
            ...
            log.info("Indexing has not been stopped because it is not running");
            return false;
        }
    }
